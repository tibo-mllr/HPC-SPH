...
def getPairwiseSeparations(cnp.ndarray[DTYPE_t, ndim=2] ri, cnp.ndarray[DTYPE_t, ndim=2] rj):
    """
    Get pairwise separations between 2 sets of coordinates
    ri is an 400 x 3 matrix of positions
    rj is an 400 x 3 matrix of positions
    """

    # Use hardcoded sizes for the arrays
    cdef cnp.ndarray[DTYPE_t, ndim=2] dx = np.empty((400, 400), dtype=np.float64)
    cdef cnp.ndarray[DTYPE_t, ndim=2] dy = np.empty((400, 400), dtype=np.float64)
    cdef cnp.ndarray[DTYPE_t, ndim=2] dz = np.empty((400, 400), dtype=np.float64)

    cdef unsigned int i, j

    # Use prange for parallel execution
    for i in prange(400, nogil=True):
        for j in range(400):
...
@cython.boundscheck(False)
def getDensity(cnp.ndarray[DTYPE_t,ndim=2] r, cnp.ndarray[DTYPE_t,ndim=2] pos, float m, float h):
    """
